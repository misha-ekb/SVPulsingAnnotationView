...
@property (nonatomic, strong) CALayer *colorHaloLayer2;

@property (nonatomic, strong) CAAnimationGroup *pulseAnimationGroup;
@property (nonatomic, strong) CAAnimationGroup *pulseAnimationGroup2;
...
- (void)rebuildLayers {
    if(self.shouldBeFlat) {
        ...
        [_colorHaloLayer2 removeFromSuperlayer];
        _colorHaloLayer2 = nil;
        
        [self.layer addSublayer:self.colorHaloLayer];
        [self.layer addSublayer:self.colorHaloLayer2];
        
        if (!self.hideCentralDot) {
...
- (CAAnimationGroup*)pulseAnimationGroup2 {
    if(!_pulseAnimationGroup2) {
        CAMediaTimingFunction *defaultCurve = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionDefault];
        
        _pulseAnimationGroup2 = [CAAnimationGroup animation];
        _pulseAnimationGroup2.duration = self.outerPulseAnimationDuration + self.delayBetweenPulseCycles;
        _pulseAnimationGroup2.repeatCount = INFINITY;
        _pulseAnimationGroup2.removedOnCompletion = NO;
        _pulseAnimationGroup2.timingFunction = defaultCurve;
        ...
        _pulseAnimationGroup2.animations = animations;
    }
    return _pulseAnimationGroup2;
}
...
- (CALayer *)colorHaloLayer {
    if(!_colorHaloLayer) {
        ...
        _colorHaloLayer.bounds = CGRectMake(0, 0, 80, 80);
        ...
        _colorHaloLayer.cornerRadius = 40;
        _colorHaloLayer.opacity = 0;
        _colorHaloLayer.borderColor = self.annotationColor.CGColor;
        _colorHaloLayer.borderWidth = 2.0;
...
- (CALayer *)colorHaloLayer2 {
    if(!_colorHaloLayer2) {
        _colorHaloLayer2 = [CALayer layer];
        _colorHaloLayer2.bounds = CGRectMake(0, 0, 120, 120);
        _colorHaloLayer2.position = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
        _colorHaloLayer2.contentsScale = [UIScreen mainScreen].scale;
        //_colorHaloLayer.backgroundColor = self.annotationColor.CGColor;
        _colorHaloLayer2.cornerRadius = 60;
        _colorHaloLayer2.opacity = 0;
        _colorHaloLayer2.borderColor = self.annotationColor.CGColor;
        _colorHaloLayer2.borderWidth = 2.0;
        
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            if(self.delayBetweenPulseCycles != INFINITY) {
                CAAnimationGroup *animationGroup = self.pulseAnimationGroup2;
                
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    [_colorHaloLayer2 addAnimation:animationGroup forKey:@"pulse"];
                });
            }
        });
    }
    return _colorHaloLayer2;
}
